...
if rank_button:
    ...
    st.caption("Showing all candidates ranked from highest to lowest fit score")
    chart_df = results_df.set_index("Filename")[["Fit Score (%)"]]
    st.bar_chart(chart_df, horizontal=True)
    
    # Keywords section
    st.markdown("### 🔑 Key Skills & Qualifications by Candidate")
    st.caption("Top keywords extracted from each resume using TF-IDF analysis")
    ...
    for _, row in results_df.iterrows():
        ...
        with st.expander(f"{emoji} Rank #{rank}: {filename} — {score:.1f}% match"):
            if keywords:
                # Display as tags
